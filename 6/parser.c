...
void assemble(const char *file_name, instruction *instructions, int num_instructions) {
    ...
    char hack_file_name[strlen(file_name) + 6];
    sprintf(hack_file_name, "%s.hack", file_name);

    FILE *hack_file = fopen(hack_file_name, "w");
    if (hack_file == NULL) {
        exit_program(EXIT_CANNOT_OPEN_FILE, hack_file_name);
    }

    hack_addr next_free_address = 16;


    for (int i = 0; i < num_instructions; i++) {
        char binary_opcode[17] = {0};
        instruction instr = instructions[i];

        if (instr.type == A_TYPE_INSTR) {
            if (instr.instr.A_TYPE_INSTR.is_addr) {
                sprintf(binary_opcode, "0%015d", instr.instr.A_TYPE_INSTR.operand.address);
            } else {
                Symbol *symbol = symtable_find(instr.instr.A_TYPE_INSTR.operand.label);
                if (symbol == NULL) {
                    symtable_insert(instr.instr.A_TYPE_INSTR.operand.label, next_free_address);
                    symbol = symtable_find(instr.instr.A_TYPE_INSTR.operand.label);
                    next_free_address++;
                }
                sprintf(binary_opcode, "0%015d", symbol->address);
            }
            free(instr.instr.A_TYPE_INSTR.operand.label);
        } else if (instr.type == C_TYPE_INSTR) {
            c_instruction *c_instr = &instr.instr.C_TYPE_INSTR;
            opcode c_opcode = 0b111 << 13;

            c_opcode |= c_instr->a << 12;
            c_opcode |= c_instr->comp << 6;
            c_opcode |= c_instr->dest << 3;
            c_opcode |= c_instr->jump;
            sprintf(binary_opcode, "%016d", c_opcode);
        }
